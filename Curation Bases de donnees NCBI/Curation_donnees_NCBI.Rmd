...
```{r}
# Remplacez toutes les modalités contenant "pig" par "pig" dans la colonne "host"
metadata_df$host <- gsub(".*Chicken.*", "Chicken", metadata_df$host, ignore.case = TRUE)

# Ajoutez une colonne "Sequence_Length" avec les 4 derniers éléments de "Location.Qualifiers"
metadata_df$Sequence_Length <- substr(metadata_df$`Location/Qualifiers`, nchar(metadata_df$`Location/Qualifiers`) - 4, nchar(metadata_df$`Location/Qualifiers`))

metadata_df$ORF = metadata_df$note
metadata_df$ORF <- gsub(".*ORF1.*", "ORF1", metadata_df$ORF, ignore.case = TRUE)
metadata_df$ORF <- gsub(".*ORF2.*", "ORF2", metadata_df$ORF, ignore.case = TRUE)
metadata_df$ORF <- gsub(".*ORF3.*", "ORF3", metadata_df$ORF, ignore.case = TRUE)

library(dplyr)
metadata_df$ORF <- ifelse(grepl("ORF", metadata_df$ORF, ignore.case = TRUE), metadata_df$ORF, NA)
```
...
HEV_metadata$Pays  = str_extract(HEV_metadata$country, "^[^:]+")
HEV_metadata$Annee <- substr(HEV_metadata$DateSoumission, 7,11)
HEV_metadata$Annee <- gsub("-", "", HEV_metadata$Annee)
```

## Dimension jeux de donnees apres nettoyage:
```{r}
dim(HEV_metadata)
length(which(is.na(HEV_metadata$genotype))) # Nombre de sequences sans genotype
